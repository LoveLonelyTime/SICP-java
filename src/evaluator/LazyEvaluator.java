...
public class LazyEvaluator implements EvaluationVisitor<Environment, Primitive> {
    ...
    private Primitive force(Primitive p) {
        return p.accept(new PrimitiveVisitor<>() {
            ...
            @Override
            public Primitive visit(SymbolPrimitive primitive) {
                return primitive;
            }
    ...
    public Primitive visit(IfEvaluation e, Environment env) {
        if (force(e.getCondition().accept(this, env)).accept(DefaultPrimitiveVisitor.expectBooleanPrimitive).getValue()) { // Force
    ...
    public Primitive visit(ProcedureEvaluation e, Environment env) {
        Primitive operator = force(e.getOperator().accept(this, env));
    ...
    public Primitive visit(AMBEvaluation e, Environment env) {
        throw new IllegalStateException("AMB operation is not supported by LazyEvaluator.");
    }
